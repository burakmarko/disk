...
void initArray()
{
    for(int i=0; i<SIZE; i++)
    {
        distance[i]=10000;        // Всі відстані поки що є
невизначеними, тому 10000
        vertex[i]=1;              // Всі вершини є необійденими, мають
значення 1
    }
...
int main(void)
{
    ...
    int minindex;                 // Змінна для ітерацій циклу
основного алгоритму
    int min;
    ...
    do
    {
        ...
        for (int i=0; i<SIZE; i++)
        {
            if ((vertex[i]==1) && (distance[i]<min))         // Якщо
вершину ще не обійшли і вага менша за min
            {
                min=distance[i];                             //
Встановлюємо змінній min мінімальне значення
                minindex=i;      // Визначаємо позицію мінімальної відстані
            }
        }

        if(min!=10000)                                // Якщо
попередня умова виконалась
        {
            for(int i=0; i<SIZE; i++)
            {
                if(Array[minindex][i]>0)
                {
                    ...
                    {
                        distance[i]=temp;                           //
Відстань записуємо в масив відстаней
                    }
                }
            }
            vertex[minindex]=0;                               //
Помічаємо вершину пройденою
        }
    }
    while(minindex < 10000);                                // Допоки
не знайдемо всіх відстаней
...
найкоротших відстаней
    ...
    while (end > 0) // Поки не дійшли до початкової вершини
    {
        for(int i=0; i<SIZE; i++) // Переглядаємо всі вершини
            ...
            {
                int temp = weight - Array[end][i]; // Визначаємо вагу
шляху з попередньої вершини
                if (temp == distance[i]) // Якщо вага співпала з вирахуваною
                {                        // Значить, з цієї вершини був
здійснений перехід
